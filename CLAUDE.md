...
7. **Backend deploys go through the `nayvert` git remote, not `origin`.** The repo has two remotes: `origin` (`an03shul/saree-wholesale-app`) and `nayvert` (`nayvertai-ctrl/saree-wholesale-app`). Railway's GitHub integration is connected to the **nayvert** repo only. `git push origin main` updates GitHub but deploys nothing — no error, no signal (Railway reports no deploy status back to the origin repo, so the commit status just stays "pending" forever). Push to **both**: `git push origin main && git push nayvert main`. Symptom when you forget: the API stays healthy but keeps serving old code indefinitely. To verify a rollout landed, don't rely on `/api/health` (it has no version marker) — probe a behavior the new code changes, or watch for the brief connection drop (~1s) at cutover.
